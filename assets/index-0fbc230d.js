...
function BrandDashboard({ data, accent, allStores, label }) {
  ...
  const CITY_NAMES = /\b(nashville|atlanta|chicago|denver|austin|orlando|dallas|houston|phoenix|charlotte|pittsburgh|indianapolis|st louis|san antonio|franklin|gatlinburg|birmingham|huntsville|raleigh|richmond|washington|new york|nyc|boston|philadelphia|seattle|portland|minneapolis|kansas city|oklahoma city|memphis|louisville|cincinnati|columbus|cleveland|detroit|miami|tampa|jacksonville|new orleans|lynnfield|times square|alexandr|tyson|berry hill|lincoln park|salt lake|las vegas|downtown|midtown|uptown|southcenter)\b/i;
  function getCategory(name) {
    const n = name || "";
    if (/cosmo plushie keychain/i.test(n)) return "Cosmo Plushie Keychain";
    ...
    if (/\bshirt\b|\btee\b|\bcosmo\s+ie\b|\bi\s+escaped\b|\bstamp\b|\bescapee\b|\bcosmo\s+youth\b/i.test(n)) return "Shirts";
    if (CITY_NAMES.test(n)) return "Shirts";
    if (/\bie\b/i.test(n) && !/\b(pie|tie|die|lie|bie|movie)\b/i.test(n)) return "Shirts";
  ...
  const byMonth = {};
  allRows.forEach(r => {
    const cat = getCategory(r.name) || r.name;
    if (!byCategory[cat]) byCategory[cat] = {name:cat,units:0,revenue:0};
  ...
  const COLORS2 = ["#FF5C1A","#D4A017","#4ECDC4","#A78BFA","#F59E0B","#34D399","#F87171","#60A5FA","#FB923C","#A3E635"];
  ...
  const chartData = months.map((mk,i) => {
    ...
    activeCats.forEach(cat => { obj[cat] = byMonth[mk]?.[cat] ? +byMonth[mk][cat].toFixed(2) : 0; });
    return obj;
  });
  ...
  return React.createElement("div", null,
    React.createElement("div", {style:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:16}},
      React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:"18px 22px",position:"relative",overflow:"hidden"}},
        React.createElement("div", {style:{position:"absolute",top:0,left:0,right:0,height:2,background:accent}}),
        React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase",marginBottom:7}}, "Revenue"),
        React.createElement("div", {style:{fontSize:24,fontFamily:"'Bebas Neue',sans-serif",color:"#fff"}}, "$" + filteredRev.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}))
      ),
      React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:"18px 22px",position:"relative",overflow:"hidden"}},
        React.createElement("div", {style:{position:"absolute",top:0,left:0,right:0,height:2,background:"#D4A017"}}),
        React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase",marginBottom:7}}, "Units Sold"),
        React.createElement("div", {style:{fontSize:24,fontFamily:"'Bebas Neue',sans-serif",color:"#fff"}}, filteredUnits.toLocaleString())
      ),
      React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:"18px 22px",position:"relative",overflow:"hidden"}},
        React.createElement("div", {style:{position:"absolute",top:0,left:0,right:0,height:2,background:"#4ECDC4"}}),
        React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase",marginBottom:7}}, "Avg / Unit"),
        React.createElement("div", {style:{fontSize:24,fontFamily:"'Bebas Neue',sans-serif",color:"#fff"}}, filteredUnits ? "$" + (filteredRev/filteredUnits).toFixed(2) : "—")
      )
    ),
    React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:16,marginBottom:16}},
      React.createElement("div", {style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
        React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,textTransform:"uppercase"}}, "Select Categories"),
      ...
      React.createElement("div", {style:{display:"flex",flexWrap:"wrap",gap:6}},
        cats.map((cat,i) => {
          const active = selCats.length===0 || selCats.includes(cat.name);
          return React.createElement("button", {
            key: cat.name,
            onClick: () => setSelCats(prev => {
              if (prev.length===0) return cats.map(c=>c.name).filter(n=>n!==cat.name);
              return prev.includes(cat.name) ? prev.filter(n=>n!==cat.name) : [...prev,cat.name];
            }),
            style:{padding:"5px 12px",borderRadius:6,background:active?COLORS2[i%10]+"33":"transparent",border:`1px solid ${active?COLORS2[i%10]:"#1E1E2E"}`,color:active?COLORS2[i%10]:"#555",fontSize:11,cursor:"pointer",fontFamily:"'DM Mono',monospace",transition:"all .15s"}
          }, cat.name);
        })
      )
    ),
    chartData.length > 0 && React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:20,marginBottom:16}},
      React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,marginBottom:14,textTransform:"uppercase"}}, "Revenue by Category Over Time"),
      React.createElement(Recharts.ResponsiveContainer, {width:"100%",height:280},
        React.createElement(Recharts.BarChart, {data:chartData},
          ...
          React.createElement(Recharts.Tooltip, {contentStyle:{background:"#1A1A2E",border:"1px solid #1E1E2E",borderRadius:8,fontFamily:"'DM Mono',monospace",fontSize:11}}),
          React.createElement(Recharts.Legend, {formatter:v=>React.createElement("span",{style:{color:"#888",fontSize:10}},v)}),
          ...activeCats.map((cat,i) => React.createElement(Recharts.Bar, {key:cat,dataKey:cat,stackId:"a",fill:COLORS2[i%10],radius:i===activeCats.length-1?[3,3,0,0]:[0,0,0,0]}))
        )
      )
    ),
    React.createElement("div", {style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:20}},
      React.createElement("div", {style:{fontSize:10,color:"#555",letterSpacing:2,marginBottom:12,textTransform:"uppercase"}}, "Category Breakdown"),
      React.createElement("table", {style:{width:"100%",borderCollapse:"collapse",fontSize:11}},
        React.createElement("thead", null,
          React.createElement("tr", {style:{borderBottom:"1px solid #1E1E2E"}},
            ["#","Category","Units","Revenue","Avg/Unit","Share"].map(h=>
              React.createElement("th", {key:h,style:{textAlign:"left",padding:"7px 10px",color:"#444",fontWeight:400,fontSize:9,letterSpacing:1,textTransform:"uppercase"}}, h)
            )
          )
        ),
        React.createElement("tbody", null,
          filteredCats.map((cat,i) =>
            React.createElement("tr", {key:cat.name,style:{borderBottom:"1px solid #1E1E2E22"}},
              React.createElement("td", {style:{padding:"8px 10px",color:"#333"}}, i+1),
              React.createElement("td", {style:{padding:"8px 10px",display:"flex",alignItems:"center",gap:8}},
                React.createElement("div", {style:{width:8,height:8,borderRadius:"50%",background:COLORS2[cats.findIndex(c=>c.name===cat.name)%10],flexShrink:0}}),
                React.createElement("span", {style:{color:"#ddd"}}, cat.name)
              ),
              React.createElement("td", {style:{padding:"8px 10px",color:"#4ECDC4"}}, cat.units.toLocaleString()),
              React.createElement("td", {style:{padding:"8px 10px",color:accent}}, "$"+cat.revenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})),
              React.createElement("td", {style:{padding:"8px 10px",color:"#D4A017"}}, cat.units?"$"+(cat.revenue/cat.units).toFixed(2):"—"),
              React.createElement("td", {style:{padding:"8px 10px",minWidth:100}},
                React.createElement("div", {style:{display:"flex",alignItems:"center",gap:6}},
                  React.createElement("div", {style:{flex:1,height:3,background:"#1E1E2E",borderRadius:2}},
                    React.createElement("div", {style:{width:(totalRev?cat.revenue/totalRev*100:0)+"%",height:"100%",background:COLORS2[cats.findIndex(c=>c.name===cat.name)%10],borderRadius:2}})
                  ),
