...
function App(){
  ...
  const fetchData=async(url, silentIfFail=false)=>{
    ...
    try{
      ...
      if(slim){
        const d=expand(slim);
        // Only update if Sheets has MORE data than local (row count check)
        const localRows=(getLocal()?.revelRows?.length||0)+(getLocal()?.resovaRows?.length||0);
        const sheetsRows=(d.revelRows?.length||0)+(d.resovaRows?.length||0);
        if(sheetsRows>=localRows){setRawData(d);saveLocal(d);}
