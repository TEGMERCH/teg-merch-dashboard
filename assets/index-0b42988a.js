...
function csvToRevelRows(rows, store, fallbackDate = "") {
  ...
  return rows.filter((row) => {
    ...
    if (/gift|trivia showdown|holiday edition|single contestant|private studio|4for100|4 for 100|4 for \$100|launch|great big game show|the original|reservation id/i.test(rawName)) return false;
    return true;
  }).map((row) => {
...
function csvToResovaRows(rows, store, filenameBrand=null) {
  ...
  return cleanRows.filter((r) => {
    ...
    if (/gift card|single.?player|private experience|private studio|trivia showdown|holiday edition|single contestant|4for100|4 for 100|4 for \$100|launch|great big game show|the original|reservation id/i.test(item)) return false;
