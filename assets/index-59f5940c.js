...
function xlsxToXolaRows(workbook, store) {
  ...
  for (let i = 2; i < raw.length; i++) {
    ...
    if (itemsInRow.length === 0) {
      ...
      const totalQty = itemsInRow.reduce((s, c) => s + (parseFloat(row[c.idx]) || 0), 0);
      const revenuePerUnit = totalQty > 0 ? revenue / totalQty : 0;
      for (const col of itemsInRow) {
        const qty = parseFloat(row[col.idx]) || 0;
        const name = isShirtRow ? productClean : normalizeName(col.name);
        if (/gift/i.test(name)) continue;
        result.push({
          date: dateStr,
          name,
          category: isShirtRow ? "Apparel" : "Merch",
          units: qty,
          revenue: +(revenuePerUnit * qty).toFixed(2),
          store: rowStore,
          brand
        });
      }
