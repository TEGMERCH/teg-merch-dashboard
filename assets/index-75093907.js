...
function saveLocal(d) { try{localStorage.setItem(DATA_KEY,JSON.stringify(d));}catch(e){console.error("saveLocal failed:",e);} }
...
async function saveToSheets(url, data) {
  // Full save: clear sheet then write all rows in chunks
...
async function appendToSheets(url, newRR, newSR, ts) {
  // Append-only: just adds new rows, never touches existing ones
  const CHUNK=200;
...
function csvToRevelRows(rows, store, fallbackDate="") {
  ...
  return rows
    ...
    // Fallback to filename date if no date found in row
...
function xlsxToXolaRows(workbook, store) {
  ...
  for (let i = 2; i < raw.length; i++) {
    ...
    // Col C (idx 2) = Product — must contain "merch" to be included
    const product    = (row[2] || "").toString();
    if (!/merch/i.test(product)) continue;
    // Col F (idx 5) = Customer/Name — this is the actual product name
    ...
    // Brand comes from Col C (Product) — contains "Great Big Game", "Adventure Mining", etc.
    ...
    let date = "";
    if (arrivalDate) { date = parseDate(arrivalDate); }
...
function aggregate(revelRows, resovaRows, stores, dateFrom, dateTo) {
  ...
  const toMonthKey=d=>{
    if(!d) return null;
    // Handle date strings in various formats: "2026-02-15", "13-Feb-2026", JS Date serial numbers
  ...
  rr.forEach(r=>{
    ...
    byMonth[mk].revenue+=r.revenue; byMonth[mk].units+=r.units; byMonth[mk].transactions++;
  ...
  sr.forEach(r=>{
    ...
    if(!byStore[r.store])  byStore[r.store]  ={store:r.store,revenue:0,units:0};
    byMonth[mk].revenue+=r.revenue; byMonth[mk].units+=units; byMonth[mk].transactions++;
  ...
  return{
    ...
    monthlyRevenue:Object.values(byMonth).sort((a,b)=>a.key.localeCompare(b.key)).map(m=>({...m,revenue:+m.revenue.toFixed(2)})),
    topItems,
      ...
      {subtab==="search"&&<div className="fu">
        ...
        {(()=>{
          ...
          [...matchRevel,...matchResova.map(r=>({...r,name:r.item,units:r.qty||1}))].forEach(r=>{
            const month=r.date?(()=>{const mk=String(r.date).trim();const iso=/^\d{4}-\d{2}/.test(mk)?mk.slice(5,7):null;const dt=iso?null:new Date(mk);const mi=iso?parseInt(iso)-1:isNaN(dt)?-1:dt.getMonth();return mi>=0?MONTHS[mi]:"?";})():"?";
          ...
          return(<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:16}}>
              ...
              <KPI label="Avg / Unit" value={totalUnits?`$${(totalRev/totalUnits).toFixed(2)}`:"—"} color={PURPLE} icon="🏷"/>
            </div>
        ...
        {(()=>{
          ...
          const calcPeriod=(from,to)=>{
            ...
            [...rr,...sr.map(r=>({...r,name:r.item,units:r.qty||1}))].forEach(r=>{
              const m=r.date?(()=>{const mk=String(r.date).trim();const iso=/^\d{4}-\d{2}/.test(mk)?mk.slice(5,7):null;const dt=iso?null:new Date(mk);const mi=iso?parseInt(iso)-1:isNaN(dt)?-1:dt.getMonth();return mi>=0?MONTHS[mi]:"?";})():"?";
...
function UploadModal({sheetsUrl,existingData,onSave,onClose}){
  ...
  const upload=async()=>{
    try{
      ...
      // localStorage is always the most complete copy — use it as the base
      const base=getLocal()||existingData||{revelRows:[],resovaRows:[]};

      // Parse only the NEW rows from uploaded files
      ...
      // Save locally FIRST — localStorage is the source of truth
      saveLocal(newData);
      setStatus("saving");

      // Sync only the NEW rows to Sheets (append-only, never wipes)
      if(sheetsUrl){
        try{
        ...
        catch(e){ setError(`Saved locally but Sheets sync failed: ${e.message}`); }
      }
      onSave(newData);
...
function App(){
  ...
  useEffect(()=>{
    ...
    // Always show local data immediately if we have it
    if(local){setRawData(local);}
    // Then try to sync from Sheets in background (don't block UI)
    if(url){
      fetchData(url, !!local);
    } else if(!local){
      setShowSetup(true);
  ...
  const fetchData=async(url, silentIfFail=false)=>{
    ...
    try{
      ...
      if(slim){
        ...
        const localRows=(localData?.revelRows?.length||0)+(localData?.resovaRows?.length||0);
        const sheetsRows=(d.revelRows?.length||0)+(d.resovaRows?.length||0);
        if(!localData){
          // No local data at all — use Sheets data
          setRawData(d);saveLocal(d);
        } else if(sheetsRows>localRows){
          // Sheets has MORE rows — use Sheets (another device may have added data)
          setRawData(d);saveLocal(d);
        }
        // If local has same or more rows, keep local — don't let Sheets overwrite it
      } else if(!silentIfFail){
        setShowUpload(true);
      }
    }
    catch(e){if(!silentIfFail)setLoadErr(e.message);}
