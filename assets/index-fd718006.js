var _app = (() => {
  ...
  function saveLocal(d) {
    try {
      ...
      const check = localStorage.getItem(DATA_KEY);
      if (!check) throw new Error("Item was not saved \u2014 localStorage may be blocked");
      return true;
    } catch (e) {
  ...
  function xlsxToXolaRows(workbook, store) {
    ...
    for (let i = 2; i < raw.length; i++) {
      ...
      if (/gift/i.test(customerName)) continue;
      ...
      let date = "";
      if (arrivalDate) {
        date = parseDate(arrivalDate);
      }
      result.push({ date, name, category: "Merch", units: 1, revenue, store: rowStore, brand });
    }
    return result;
  }
  ...
  function BrandDashboard({ data, accent, allStores, label }) {
    ...
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 0, borderBottom: `1px solid ${BORDER}` } }, SUBTABS.map((t) => /* @__PURE__ */ React.createElement("button", { key: t.id, onClick: () => setSubtab(t.id), style: { padding: "7px 14px", background: "transparent", border: "none", borderBottom: `2px solid ${subtab === t.id ? accent : "transparent"}`, color: subtab === t.id ? accent : "#555", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 1, cursor: "pointer", marginBottom: -1 } }, t.label))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" } }, allStores.length > 1 && /* @__PURE__ */ React.createElement(StorePicker, { allStores, selected: selStores, onChange: setSelStores, accent }), /* @__PURE__ */ React.createElement(DateRange, { revelRows: data.revelRows, resovaRows: data.resovaRows, dateFrom, dateTo, onChange: (f, t) => {
      ...
      [...matchRevel, ...matchResova.map((r) => __spreadProps(__spreadValues({}, r), { name: r.item, units: r.qty || 1 }))].forEach((r) => {
        const month = r.date ? (() => {
          const mk = String(r.date).trim();
          const iso = /^\d{4}-\d{2}/.test(mk) ? mk.slice(5, 7) : null;
          const dt = iso ? null : new Date(mk);
          const mi = iso ? parseInt(iso) - 1 : isNaN(dt) ? -1 : dt.getMonth();
          return mi >= 0 ? MONTHS[mi] : "?";
        })() : "?";
        if (!byMonth[month]) byMonth[month] = { month, revenue: 0, units: 0 };
        byMonth[month].revenue += r.revenue;
        byMonth[month].units += r.units || 1;
      });
      const monthData = MONTHS.filter((m) => byMonth[m]).map((m) => __spreadProps(__spreadValues({}, byMonth[m]), { revenue: +byMonth[m].revenue.toFixed(2) }));
      const byStore = {};
      [...matchRevel, ...matchResova.map((r) => __spreadProps(__spreadValues({}, r), { name: r.item, units: r.qty || 1 }))].forEach((r) => {
        if (!byStore[r.store]) byStore[r.store] = { store: r.store, revenue: 0, units: 0 };
        byStore[r.store].revenue += r.revenue;
      ...
      const calcPeriod = (from, to) => {
        ...
        [...rr, ...sr.map((r) => __spreadProps(__spreadValues({}, r), { name: r.item, units: r.qty || 1 }))].forEach((r) => {
          ...
          byMonth[m].units += r.units || 1;
        });
        const monthly = MONTHS.filter((m) => byMonth[m]).map((m) => __spreadProps(__spreadValues({}, byMonth[m]), { revenue: +byMonth[m].revenue.toFixed(2) }));
      ...
      })), (a.monthly.length > 0 || b.monthly.length > 0) && /* @__PURE__ */ React.createElement("div", { style: { background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 20, marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 14, textTransform: "uppercase" } }, "Revenue Over Time \u2014 Period A vs Period B"), /* @__PURE__ */ React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, /* @__PURE__ */ React.createElement(LineChart, null, /* @__PURE__ */ React.createElement(CartesianGrid, { strokeDasharray: "3 3", stroke: "#1A1A2A" }), /* @__PURE__ */ React.createElement(XAxis, { dataKey: "month", type: "category", allowDuplicatedCategory: false, tick: { fill: "#555", fontSize: 10 }, axisLine: false, tickLine: false }), /* @__PURE__ */ React.createElement(YAxis, { tick: { fill: "#555", fontSize: 10 }, axisLine: false, tickLine: false, tickFormatter: (v) => `$${(v / 1e3).toFixed(1)}K` }), /* @__PURE__ */ React.createElement(Tooltip, { content: /* @__PURE__ */ React.createElement(Tip, null) }), a.monthly.length > 0 && /* @__PURE__ */ React.createElement(Line, { data: a.monthly, type: "monotone", dataKey: "revenue", stroke: accent, strokeWidth: 2, dot: { fill: accent, r: 3 }, name: "Period A Revenue" }), b.monthly.length > 0 && /* @__PURE__ */ React.createElement(Line, { data: b.monthly, type: "monotone", dataKey: "revenue", stroke: "#888", strokeWidth: 2, dot: { fill: "#888", r: 3 }, name: "Period B Revenue", strokeDasharray: "5 5" }))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 16, marginTop: 8, justifyContent: "center" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 2, background: accent } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#666" } }, "Period A")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 20, height: 2, background: "#888", borderTop: "2px dashed #888" } }), /* @__PURE__ */ React.createElement("span", { style: { fontSize: 10, color: "#666" } }, "Period B")))), /* @__PURE__ */ React.createElement("div", { style: { background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 20 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, marginBottom: 14, textTransform: "uppercase" } }, "Product Breakdown \u2014 Period A vs Period B"), /* @__PURE__ */ React.createElement("table", { style: { width: "100%", borderCollapse: "collapse", fontSize: 11 } }, /* @__PURE__ */ React.createElement("thead", null, /* @__PURE__ */ React.createElement("tr", { style: { borderBottom: `1px solid ${BORDER}` } }, ["Product", "A Units", "B Units", "Unit \u0394%", "A Revenue", "B Revenue", "Rev \u0394%"].map((h) => /* @__PURE__ */ React.createElement("th", { key: h, style: { textAlign: "left", padding: "7px 10px", color: "#444", fontWeight: 400, fontSize: 9, letterSpacing: 1, textTransform: "uppercase", whiteSpace: "nowrap" } }, h)))), /* @__PURE__ */ React.createElement("tbody", null, allProds.map((name) => {
  ...
  function UploadModal({ sheetsUrl, existingData, onSave, onClose }) {
    ...
    const upload = async () => {
      try {
        ...
        const base = getLocal() || existingData || { revelRows: [], resovaRows: [] };
        ...
        setStatus("saving");
        const saved = saveLocal(newData);
        if (!saved) {
          throw new Error("localStorage save failed. In Chrome, go to Settings > Privacy and Security > Cookies and check that theescapekeys.github.io is not blocked.");
        }
        if (sheetsUrl) {
          try {
            ...
            setError(`Saved locally but Sheets sync failed: ${e.message}`);
          }
        }
        onSave(newData);
      } catch (e) {
  ...
  function App() {
    ...
    const fetchData = async (url, silentIfFail = false) => {
      var _a2, _b2, _c, _d;
      if (!silentIfFail) setLoading(true);
      setLoadErr(null);
      try {
        ...
        if (slim) {
          ...
          const localRows = (((_a2 = localData == null ? void 0 : localData.revelRows) == null ? void 0 : _a2.length) || 0) + (((_b2 = localData == null ? void 0 : localData.resovaRows) == null ? void 0 : _b2.length) || 0);
          const sheetsRows = (((_c = d.revelRows) == null ? void 0 : _c.length) || 0) + (((_d = d.resovaRows) == null ? void 0 : _d.length) || 0);
          if (!localData) {
            setRawData(d);
            saveLocal(d);
          } else if (sheetsRows > localRows) {
            setRawData(d);
            saveLocal(d);
          }
        } else if (!silentIfFail) {
          setShowUpload(true);
        }
      } catch (e) {
        if (!silentIfFail) setLoadErr(e.message);
      }
      if (!silentIfFail) setLoading(false);
    };
