...
function csvToRevelRows(rows, store, fallbackDate = "") {
  ...
  return rows.filter((row) => {
    ...
    const discountAmt = +(grossAmt - rev).toFixed(2);
    return { date, name, category: cat, units, revenue: +rev.toFixed(2), gross: +grossAmt.toFixed(2), discount: discountAmt > 0 ? discountAmt : 0, store: rowStore, brand };
  });
}
function csvToResovaRows(rows, store) {
  ...
  return cleanRows.filter((r) => {
    ...
    if (/gift card|single.?player|private experience/i.test(item)) return false;
    if (!item.trim()) return false;
    return true;
  }).map((row) => {
    const rawItem = f.item ? row[f.item] || "" : "";
    const rawStore = f.storeCol ? row[f.storeCol] || "" : "";
    const brand = getRowBrand(rawItem, rawItem);
    ...
    const discountAmt = parseFloat(f.discount ? row[f.discount] : 0) || 0;
    const rev = parseFloat(f.total ? row[f.total] : (gross - discountAmt)) || 0;
...
function aggregate(revelRows, resovaRows, stores, dateFrom, dateTo) {
  ...
  const totalDiscount = +[...rr, ...sr].reduce((s, r) => s + (r.discount||0), 0).toFixed(2);
...
function BrandDashboard({ data, accent, allStores, label }) {
  ...
  function getCategory(name) {
    ...
    return "Misc/Beverages";
  }

  const COLORS2 = ["#FF5C1A","#D4A017","#4ECDC4","#A78BFA","#F59E0B","#34D399","#F87171","#60A5FA","#FB923C","#A3E635","#E879F9","#67E8F9"];
  const CAT_ORDER = ["Shirts","Pins","Pin Packs","Hats","Plushies","Cosmo Plushie Keychain","Cosmo Bottle","Beanies","Shoe Charms","Unlockeds","EFIGs","Misc/Beverages"];
  ...
  const allRows = [...(data.revelRows||[]), ...(data.resovaRows||[]).map(r=>({...r,name:r.item}))];
    ...
    React.createElement("div",{style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:20}},
      ...
      React.createElement("div",{style:{display:"flex",gap:12,marginBottom:16,flexWrap:"wrap"}},
        ...
        React.createElement("div",{style:{flex:1,minWidth:260,background:"#0A0A0F",borderRadius:8,padding:"12px 14px",border:"1px solid #1E1E2E"}},
          React.createElement("div",{style:{fontSize:9,color:accent,letterSpacing:2,textTransform:"uppercase",marginBottom:8,fontFamily:"'DM Mono',monospace"}},"Period A"),
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8}},
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"From",
            React.createElement("input",{type:"date",value:periodA.from,onChange:e=>setPeriodA(p=>({...p,from:e.target.value})),style:{background:"#111118",border:"1px solid #1E1E2E",color:"#fff",borderRadius:6,padding:"5px 8px",fontSize:11,fontFamily:"'DM Mono',monospace",flex:1}}),
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"To",
            React.createElement("input",{type:"date",value:periodA.to,onChange:e=>setPeriodA(p=>({...p,to:e.target.value})),style:{background:"#111118",border:"1px solid #1E1E2E",color:"#fff",borderRadius:6,padding:"5px 8px",fontSize:11,fontFamily:"'DM Mono',monospace",flex:1}}),
        ...
        /* Period B */
        React.createElement("div",{style:{flex:1,minWidth:260,background:"#0A0A0F",borderRadius:8,padding:"12px 14px",border:"1px solid #1E1E2E"}},
          React.createElement("div",{style:{fontSize:9,color:"#D4A017",letterSpacing:2,textTransform:"uppercase",marginBottom:8,fontFamily:"'DM Mono',monospace"}},"Period B"),
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:8}},
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"From",
            React.createElement("input",{type:"date",value:periodB.from,onChange:e=>setPeriodB(p=>({...p,from:e.target.value})),style:{background:"#111118",border:"1px solid #1E1E2E",color:"#fff",borderRadius:6,padding:"5px 8px",fontSize:11,fontFamily:"'DM Mono',monospace",flex:1}}),
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"To",
            React.createElement("input",{type:"date",value:periodB.to,onChange:e=>setPeriodB(p=>({...p,to:e.target.value})),style:{background:"#111118",border:"1px solid #1E1E2E",color:"#fff",borderRadius:6,padding:"5px 8px",fontSize:11,fontFamily:"'DM Mono',monospace",flex:1}}),
      ...
      (() => {
        ...
        function periodStats(from, to) {
          const inR = d => { if(!d) return true; if(from&&d<from) return false; if(to&&d>to) return false; return true; };
          const byCat = {};
          allBaseRows.forEach(r => {
            if(!inR(r.date)) return;
            const cat = getCategory(r.name);
            if(!byCat[cat]) byCat[cat] = {units:0,revenue:0,gross:0,discount:0};
            byCat[cat].units += (r.units||r.qty||1);
            byCat[cat].revenue += (r.revenue||0);
            byCat[cat].gross += (r.gross||r.revenue||0);
            byCat[cat].discount += (r.discount||0);
          });
          return byCat;
        }

        const statsA = hasPeriods ? periodStats(periodA.from||null, periodA.to||null) : null;
        const statsB = hasPeriods ? periodStats(periodB.from||null, periodB.to||null) : null;
        const displayCats = hasPeriods
          ? [...new Set([...Object.keys(statsA||{}), ...Object.keys(statsB||{})])].filter(c=>(statsA?.[c]?.revenue||0)+(statsB?.[c]?.revenue||0)>0)
          : cats.map(c=>c.name);
        const totalRevA = hasPeriods ? Object.values(statsA||{}).reduce((s,c)=>s+c.revenue,0) : totalRev;
        const totalRevB = hasPeriods ? Object.values(statsB||{}).reduce((s,c)=>s+c.revenue,0) : 0;

        function pctBadge(a, b) {
          if(!b||!a) return null;
          const d = (a-b)/b*100;
          return React.createElement("span",{style:{fontSize:10,color:d>=0?"#22C55E":"#ef4444",marginLeft:5,whiteSpace:"nowrap"}}, (d>=0?"▲":"▼")+Math.abs(d).toFixed(1)+"%");
        }

        const TH = ({children,align="left"}) => React.createElement("th",{style:{textAlign:align,padding:"7px 8px",color:"#444",fontWeight:400,fontSize:9,letterSpacing:1,textTransform:"uppercase",whiteSpace:"nowrap",borderBottom:"1px solid #1E1E2E"}},children);
        const TD = ({children,color="#ddd",align="left"}) => React.createElement("td",{style:{padding:"8px 8px",color,textAlign:align,verticalAlign:"middle"}},children);

        return React.createElement("div",null,
          /* Header */
          React.createElement("div",{style:{fontSize:10,color:"#555",letterSpacing:2,marginBottom:12,textTransform:"uppercase"}},
            hasPeriods?"Category Breakdown — Period A vs Period B":"Category Breakdown"
          ),
            ...
            React.createElement("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:11}},
              React.createElement("thead",null,
                React.createElement("tr",null,
                  ...
                  ...(hasPeriods ? [
                    /* Period A columns */
                    React.createElement("th",{key:"a-head",colSpan:5,style:{textAlign:"center",padding:"5px 8px",color:accent,fontWeight:500,fontSize:9,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #1E1E2E",borderLeft:"1px solid #1E1E2E"}},"Period A "+(periodA.from?periodA.from+" → "+(periodA.to||"now"):"")),
                    /* Period B columns */
                    React.createElement("th",{key:"b-head",colSpan:6,style:{textAlign:"center",padding:"5px 8px",color:"#D4A017",fontWeight:500,fontSize:9,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #1E1E2E",borderLeft:"1px solid #1E1E2E"}},"Period B "+(periodB.from?periodB.from+" → "+(periodB.to||"now"):"")+" vs A"),
                  ] : [
                    TH({children:"Units"}),
                    TH({children:"Gross",align:"right"}),
                    TH({children:"Actual Rev",align:"right"}),
                    TH({children:"Avg/Unit",align:"right"}),
                    TH({children:"Share",align:"right"}),
                  ])
                ),
                hasPeriods && React.createElement("tr",null,
                  React.createElement("th",{style:{padding:"4px 8px",borderBottom:"1px solid #1E1E2E"}}),
                  React.createElement("th",{style:{padding:"4px 8px",borderBottom:"1px solid #1E1E2E"}}),
                  /* A sub-headers */
                  ...[["Units","#4ECDC4"],["Gross","#22C55E"],["Actual","inherit"],["Avg","#D4A017"],["Share","#555"]].map(([l,c],i)=>
                    React.createElement("th",{key:"ah"+i,style:{textAlign:"right",padding:"4px 8px",color:c,fontWeight:400,fontSize:9,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #1E1E2E",borderLeft:i===0?"1px solid #1E1E2E":"none"}}),l
                  ),
                  /* B sub-headers */
                  ...[["Units","#4ECDC4"],["Gross","#22C55E"],["Actual",accent],["Avg","#D4A017"],["Share","#555"],["Δ","#888"]].map(([l,c],i)=>
                    React.createElement("th",{key:"bh"+i,style:{textAlign:"right",padding:"4px 8px",color:c,fontWeight:400,fontSize:9,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #1E1E2E",borderLeft:i===0?"1px solid #1E1E2E":"none"}}),l
                  )
                )
              ),
              React.createElement("tbody",null,
                displayCats.map((catName) => {
                  const colorIdx = cats.findIndex(c=>c.name===catName);
                  const color = COLORS2[(colorIdx<0?0:colorIdx)%12];
                  const catObj = cats.find(c=>c.name===catName);
                  const isOpen = (expandedCats||[]).includes(catName);
                  const itemList = catObj ? Object.entries(catObj.items).sort((a,b)=>b[1].revenue-a[1].revenue) : [];
                  const colSpanFull = hasPeriods ? 13 : 7;

                  const a = statsA?.[catName]||{units:0,revenue:0,gross:0};
                  const b = statsB?.[catName]||{units:0,revenue:0,gross:0};
                  const aAvg = a.units ? a.revenue/a.units : 0;
                  const bAvg = b.units ? b.revenue/b.units : 0;
                  const aShare = totalRevA ? (a.revenue/totalRevA*100).toFixed(1)+"%" : "0%";
                  const bShare = totalRevB ? (b.revenue/totalRevB*100).toFixed(1)+"%" : "0%";

                  // Non-period row data
                  const base = catObj || {units:0,revenue:0,gross:0,discount:0};

                  return [
                    React.createElement("tr",{key:catName,style:{borderBottom:isOpen?"none":"1px solid #1E1E2E22",cursor:"pointer"},onClick:()=>setExpandedCats(prev=>(prev||[]).includes(catName)?(prev||[]).filter(n=>n!==catName):[...(prev||[]),catName])},
                      TD({children:React.createElement("span",{style:{color,fontSize:12}}),isOpen?"▼":"▶"}),
                      TD({children:React.createElement("span",{style:{color:"#ddd",fontWeight:500}}),catName}),
                      ...(hasPeriods ? [
                        /* Period A cells */
                        TD({children:React.createElement("span",{style:{color:"#4ECDC4"}}),a.units.toLocaleString(),align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#22C55E"}}), a.gross>0?"$"+a.gross.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—",align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#ddd"}}), a.revenue>0?"$"+a.revenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—",align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#D4A017"}}), aAvg?"$"+aAvg.toFixed(2):"—",align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#555"}}),aShare,align:"right"}),
                        /* Period B cells */
                        TD({children:React.createElement("span",null,React.createElement("span",{style:{color:"#4ECDC4"}}),b.units.toLocaleString(),pctBadge(b.units,a.units)),align:"right"}),
                        TD({children:React.createElement("span",null,React.createElement("span",{style:{color:"#22C55E"}}), b.gross>0?"$"+b.gross.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—",pctBadge(b.gross,a.gross)),align:"right"}),
                        TD({children:React.createElement("span",null,React.createElement("span",{style:{color:accent}}), b.revenue>0?"$"+b.revenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—",pctBadge(b.revenue,a.revenue)),align:"right"}),
                        TD({children:React.createElement("span",null,React.createElement("span",{style:{color:"#D4A017"}}), bAvg?"$"+bAvg.toFixed(2):"—",pctBadge(bAvg,aAvg)),align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#555"}}),bShare,align:"right"}),
                        TD({children: (() => { const d=a.revenue&&b.revenue?(b.revenue-a.revenue)/a.revenue*100:null; return d===null?React.createElement("span",{style:{color:"#333"}}),"—":React.createElement("span",{style:{color:d>=0?"#22C55E":"#ef4444",fontWeight:500}}),(d>=0?"▲":"▼")+Math.abs(d).toFixed(1)+"%"; })(),align:"right"}),
                      ] : [
                        TD({children:React.createElement("span",{style:{color:"#4ECDC4"}}),base.units.toLocaleString(),align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#22C55E"}}), base.gross>0?"$"+(base.gross).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}):"—",align:"right"}),
                        TD({children:React.createElement("span",{style:{color:accent}}), "$"+(base.revenue||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}),align:"right"}),
                        TD({children:React.createElement("span",{style:{color:"#D4A017"}}), base.units?"$"+(base.revenue/base.units).toFixed(2):"—",align:"right"}),
                        TD({children:React.createElement("div",{style:{display:"flex",alignItems:"center",gap:6,justifyContent:"flex-end"}},
                    ...
                    isOpen && React.createElement("tr",{key:catName+"-items"},
                      React.createElement("td",{colSpan:colSpanFull,style:{padding:"0 0 10px 28px"}},
                        React.createElement("div",{style:{background:"#0A0A0F",borderRadius:8,padding:"8px 12px",borderLeft:`2px solid ${color}`}},
                          ...
                          itemList.map(([itemName,d])=>
                            React.createElement("div",{key:itemName,style:{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid #1A1A2A"}},
                              React.createElement("span",{style:{color:"#888",fontSize:11}}),itemName,
