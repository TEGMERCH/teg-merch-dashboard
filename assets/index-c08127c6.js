...
async function saveToSheets(url, data) {
  ...
  // If it fits in one request, send it directly
  if((url+"?action=save&payload="+encoded).length<=7000)
    return sheetsCall(url,{action:"save",payload:encoded});
  // Otherwise: send in chunks of 50 rows using append
  // First save clears and sets metadata with empty rows
