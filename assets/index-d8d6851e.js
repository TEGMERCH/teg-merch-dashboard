...
function csvToResovaRows(rows, store) {
  ...
  const f = {
    ...
    subtotal: fieldFind(h, "subtotal"),
    discount: fieldFind(h, "discount codes value", "discount amount", "discount", "coupon amount"),
    total:    fieldFind(h, "booking net total", "net total", "sale net total", "total", "amount", "price", "paid", "revenue"),
  ...
  return cleanRows.filter((r) => {
    ...
    // Filter gift cards, single-player, private experience
    ...
    const rawStore = f.storeCol ? row[f.storeCol] || "" : "";
    // Brand from prefix (TEG -, GBGS -, AM -) before cleaning
    ...
    return { date, item, qty, revenue: +rev.toFixed(2), gross: +gross.toFixed(2), discount: +discountAmt.toFixed(2), store: rowStore, brand };
  });
}
...
function BrandDashboard({ data, accent, allStores, label }) {
  ...
  return React.createElement("div", null,
    ...
    chartData.length>0 && (() => {
      // Build compare chart data aligned to same month labels
      const cmpChartData = cmpStats ? months.map((mk,i) => {
        const obj = {month: monthLabels[i]};
        activeCats.forEach(cat => { obj[cat] = cmpStats.byMonth[mk]?.[cat] ? +cmpStats.byMonth[mk][cat].toFixed(2) : 0; });
        return obj;
      }) : [];

      // Build gross chart data
      const grossByMonth = {};
      ...
      const grossChartData = months.map((mk,i) => {
        const obj = {month: monthLabels[i]};
        activeCats.forEach(cat => { obj[cat] = grossByMonth[mk]?.[cat] ? +grossByMonth[mk][cat].toFixed(2) : 0; });
        return obj;
      });

      // Pct change per category between main and compare
      const catChanges = cmpStats ? activeCats.map(cat => {
        const main = filteredCats.find(c=>c.name===cat);
        const cmp = cmpStats.byCategory[cat];
        if(!cmp?.revenue||!main?.revenue) return {cat,pct:null};
        const d = (main.revenue - cmp.revenue)/cmp.revenue*100;
        return {cat, pct:d.toFixed(1), up: d>=0};
      }) : [];
      ...
      return React.createElement("div",{style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:20,marginBottom:16}},
        ...
        /* % change badges when compare is set */
        cmpStats && catChanges.length>0 && React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}},
          catChanges.map(({cat,pct,up},i) => pct===null?null:
            React.createElement("div",{key:cat,style:{display:"flex",alignItems:"center",gap:4,background:"#0A0A0F",borderRadius:5,padding:"3px 8px",border:`1px solid ${up?"#22C55E22":"#ef444422"}`}},
              React.createElement("div",{style:{width:8,height:8,borderRadius:"50%",background:COLORS2[cats.findIndex(c=>c.name===cat)%12]}}),
              React.createElement("span",{style:{fontSize:10,color:"#666"}}),
              cat,
              React.createElement("span",{style:{fontSize:11,color:up?"#22C55E":"#ef4444",marginLeft:4}}, (up?"▲":"▼")+Math.abs(pct)+"%")
            )
          )
        ),
        ...
        React.createElement(Recharts.ResponsiveContainer,{width:"100%",height:320},
          chartMode==="line"
            ? React.createElement(Recharts.LineChart,{data:chartData},
                ...
                ...activeCats.map((cat,i)=>React.createElement(Recharts.Line,{key:"actual-"+cat,type:"monotone",dataKey:cat,data:chartData,stroke:COLORS2[cats.findIndex(c=>c.name===cat)%12],strokeWidth:2,dot:{r:3},activeDot:{r:5},connectNulls:false,name:cat})),
                /* Gross lines — same color, dashed */
                ...( showGross ? activeCats.map((cat,i)=>React.createElement(Recharts.Line,{key:"gross-"+cat,type:"monotone",dataKey:cat,data:grossChartData,stroke:COLORS2[cats.findIndex(c=>c.name===cat)%12],strokeWidth:1.5,strokeDasharray:"5 3",dot:false,activeDot:false,connectNulls:false,name:cat+" (Gross)",legendType:"none"})) : []),
                /* Compare period lines — same color, dotted */
                ...( cmpStats ? activeCats.map((cat,i)=>React.createElement(Recharts.Line,{key:"cmp-"+cat,type:"monotone",dataKey:cat,data:cmpChartData,stroke:COLORS2[cats.findIndex(c=>c.name===cat)%12],strokeWidth:1.5,strokeDasharray:"2 4",dot:false,activeDot:false,connectNulls:false,name:cat+" (Compare)",legendType:"none"})) : [])
              )
        ...
        chartMode==="line" && React.createElement("div",{style:{display:"flex",gap:16,marginTop:10,justifyContent:"center",flexWrap:"wrap"}},
          React.createElement("div",{style:{display:"flex",alignItems:"center",gap:5}},
            React.createElement("div",{style:{width:20,height:2,background:"#888"}}),
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"Actual Revenue"
          ),
          showGross && React.createElement("div",{style:{display:"flex",alignItems:"center",gap:5}},
            React.createElement("div",{style:{width:20,height:0,border:"1px dashed #888"}}),
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"Gross Revenue"
          ),
          cmpStats && React.createElement("div",{style:{display:"flex",alignItems:"center",gap:5}},
            React.createElement("div",{style:{width:20,height:0,border:"1px dotted #888"}}),
            React.createElement("span",{style:{fontSize:10,color:"#555"}}),"Compare Period"
          )
        )
      );
    })(),
...
/* Category breakdown table with expandable rows */
    React.createElement("div",{style:{background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:20}},
      React.createElement("div",{style:{fontSize:10,color:"#555",letterSpacing:2,marginBottom:12,textTransform:"uppercase"}},"Category Breakdown"+(cmpStats?" — Main vs Compare":"")),
      React.createElement("table",{style:{width:"100%",borderCollapse:"collapse",fontSize:11}},
        React.createElement("thead",null,
          React.createElement("tr",{style:{borderBottom:"1px solid #1E1E2E"}},
            (cmpStats
              ? ["","Category",
                  (dateFrom||dateTo)?(dateFrom?dateFrom.slice(0,7)+" → "+(dateTo?dateTo.slice(0,7):"now"):"Current")+" Units":"Units",
                  (cmpFrom||cmpTo)?(cmpFrom?cmpFrom.slice(0,7)+" → "+(cmpTo?cmpTo.slice(0,7):"now"):"Compare")+" Units":"Cmp Units",
                  (dateFrom||dateTo)?(dateFrom?dateFrom.slice(0,7)+" → "+(dateTo?dateTo.slice(0,7):"now"):"Current")+" Revenue":"Revenue",
                  (cmpFrom||cmpTo)?(cmpFrom?cmpFrom.slice(0,7)+" → "+(cmpTo?cmpTo.slice(0,7):"now"):"Compare")+" Revenue":"Cmp Revenue",
                  "Avg/Unit","Cmp Avg","Share"]
              : ["","Category","Units","Revenue","Avg/Unit","Share"]
            ).map(h=>React.createElement("th",{key:h,style:{textAlign:"left",padding:"7px 10px",color:"#444",fontWeight:400,fontSize:9,letterSpacing:1,textTransform:"uppercase",whiteSpace:"nowrap"}},h))
          )
        ),
        React.createElement("tbody",null,
          filteredCats.map((cat,i)=>{
            const colorIdx = cats.findIndex(c=>c.name===cat.name)%12;
            const avgUnit = cat.units?cat.revenue/cat.units:0;
            const cmpAvg = cat.cmp?.units?cat.cmp.revenue/cat.cmp.units:0;
            const isOpen = expandedCats?.includes(cat.name);
            const itemList = Object.entries(cat.items).sort((a,b)=>b[1].revenue-a[1].revenue);
            return [
              React.createElement("tr",{key:cat.name,style:{borderBottom:isOpen?"none":"1px solid #1E1E2E22",cursor:"pointer"},onClick:()=>setExpandedCats(prev=>(prev||[]).includes(cat.name)?(prev||[]).filter(n=>n!==cat.name):[...(prev||[]),cat.name])},
                React.createElement("td",{style:{padding:"9px 10px",color:COLORS2[colorIdx]}}, isOpen?"▼":"▶"),
                React.createElement("td",{style:{padding:"9px 10px",color:"#ddd",fontWeight:500}}, cat.name),
                React.createElement("td",{style:{padding:"9px 10px",color:"#4ECDC4"}}, cat.units.toLocaleString()),
                ...(cmpStats?[
                  React.createElement("td",{key:"cu",style:{padding:"9px 10px"}},
                    React.createElement("span",{style:{color:"#555"}},(cat.cmp?.units||0).toLocaleString()),
                    React.createElement(PctBadge,{a:cat.units,b:cat.cmp?.units||0})
                  )
                ]:[]),
                React.createElement("td",{style:{padding:"9px 10px",color:accent}}, "$"+cat.revenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})),
                ...(cmpStats?[
                  React.createElement("td",{key:"cr",style:{padding:"9px 10px"}},
                    React.createElement("span",{style:{color:"#555"}},"$"+(cat.cmp?.revenue||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})),
                    React.createElement(PctBadge,{a:cat.revenue,b:cat.cmp?.revenue||0})
                  )
                ]:[]),
                React.createElement("td",{style:{padding:"9px 10px",color:"#D4A017"}}, cat.units?"$"+avgUnit.toFixed(2):"—"),
                ...(cmpStats?[
                  React.createElement("td",{key:"ca",style:{padding:"9px 10px"}},
                    React.createElement("span",{style:{color:"#555"}},(cat.cmp?.units?"$"+cmpAvg.toFixed(2):"—")),
                    cat.cmp?.units && React.createElement(PctBadge,{a:avgUnit,b:cmpAvg})
                  )
                ]:[]),
                React.createElement("td",{style:{padding:"9px 10px",minWidth:100}},
                  React.createElement("div",{style:{display:"flex",alignItems:"center",gap:6}},
                    React.createElement("div",{style:{flex:1,height:3,background:"#1E1E2E",borderRadius:2}},
                      React.createElement("div",{style:{width:(totalRev?cat.revenue/totalRev*100:0)+"%",height:"100%",background:COLORS2[colorIdx],borderRadius:2}})
                    ),
                    React.createElement("span",{style:{color:"#444",fontSize:10}},totalRev?(cat.revenue/totalRev*100).toFixed(1)+"%":"0%")
                  )
                )
              ),
              isOpen && React.createElement("tr",{key:cat.name+"-items"},
                React.createElement("td",{colSpan:cmpStats?9:6,style:{padding:"0 0 8px 24px"}},
                  React.createElement("div",{style:{background:"#0A0A0F",borderRadius:8,padding:"8px 12px",borderLeft:`2px solid ${COLORS2[colorIdx]}`}},
                    React.createElement("div",{style:{fontSize:9,color:"#444",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}, `Items in ${cat.name}`),
