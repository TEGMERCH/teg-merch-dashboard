...
function normalizeStore(raw = "") {
  ...
  // Normalize GBGS SF aliases to "San Francisco"
  raw = raw.replace(/fisherman.?s?\s*wha.*/i, "").replace(/\s+$/, "").trim() || raw;
...
function csvToRevelRows(rows, store, fallbackDate = "") {
  ...
  return rows.filter((row) => {
    ...
    if (/gift|trivia showdown|holiday edition|single contestant|private studio|4for100|4 for 100|launch|great big game show/i.test(rawName)) return false;
    return true;
  }).map((row) => {
...
function csvToResovaRows(rows, store, filenameBrand=null) {
  ...
  return cleanRows.filter((r) => {
    ...
    if (/gift card|single.?player|private experience|private studio|trivia showdown|holiday edition|single contestant|4for100|4 for 100|launch|great big game show/i.test(item)) return false;
...
function aggregate(revelRows, resovaRows, stores, dateFrom, dateTo) {
  ...
  return {
    ...
    monthlyRevenue: Object.values(byMonth).sort((a, b) => a.key.localeCompare(b.key)).map((m) => ({...m, revenue: +m.revenue.toFixed(2), gross: +(m.gross||m.revenue).toFixed(2), discount: +(m.discount||0).toFixed(2)})),
    topItems,
