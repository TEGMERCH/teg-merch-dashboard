...
function csvToRevelRows(rows, store, fallbackDate = "") {
  ...
  return rows.filter((row) => {
    ...
    if (/gift|trivia showdown|holiday edition|single contestant|private studio/i.test(rawName)) return false;
    return true;
  }).map((row) => {
...
function csvToResovaRows(rows, store) {
  ...
  return cleanRows.filter((r) => {
    ...
    if (/gift card|single.?player|private experience|private studio|trivia showdown|holiday edition|single contestant/i.test(item)) return false;
