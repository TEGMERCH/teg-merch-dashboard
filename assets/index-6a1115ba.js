...
function normalizeStore(raw = "") {
  ...
  const ALIASES = {
    "dedham ma": "Dedham",
    "las vegas area 15": "Las Vegas Area",
    "nashville downtown": "Nashville",
  };
  const lower = raw.toLowerCase();
  if (ALIASES[lower]) return ALIASES[lower];

  return raw.replace(/\w+/g, (w) => ABBREVS[w.toLowerCase()] || w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
}
...
function csvToRevelRows(rows, store, fallbackDate = "") {
  ...
  return rows.filter((row) => {
    ...
    // For GBGS, collapse "San Francisco Wharf" → "San Francisco" (one GBGS SF store)
    if (brand === "gbgs" && /san francisco wharf/i.test(rowStore)) rowStore = "San Francisco";
...
function csvToResovaRows(rows, store, filenameBrand=null) {
  ...
  return cleanRows.filter((r) => {
    ...
    let rowStore = normalizeStore(rawStore) || normalizeStore(store || "") || "Unknown";
    // For GBGS, collapse "San Francisco Wharf" → "San Francisco"
    if (brand === "gbgs" && /san francisco wharf/i.test(rowStore)) rowStore = "San Francisco";
