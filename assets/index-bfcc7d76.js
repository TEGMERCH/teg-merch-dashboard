...
function normalizeName(raw = "") {
  return raw
    ...
    .replace(/\s+\d+\.\s*(xs|s|m|l|xl|xxl|2xl|3xl|4xl|small|medium|large)$/i, "")  // strips "7. 4xl" / "8. 4xl" etc
    .replace(/\s*\d+\.\s*4xl\s*$/i, "")  // explicit catch for N. 4XL
    .replace(/\s*\([^)]*\)\s*$/, "")
    .trim()
...
function normalizeStore(raw = "") {
  ...
  const ALIASES = {
    ...
    "seattle southcenter": "Seattle SC",
    "seattle downtown": "Seattle DT",
    "seattle reinvent": "Seattle DT",
    "seattle dt": "Seattle DT",
...
function csvToResovaRows(rows, store, filenameBrand=null) {
  ...
  return cleanRows.filter((r) => {
    ...
    const couponCode = discountAmt > 0 ? "Discount Applied" : "";
    let rowStore = normalizeStore(rawStore) || normalizeStore(store || "") || "Unknown";
    // GBGS store normalizations
    if (brand === "gbgs") {
      if (/san francisco/i.test(rowStore)) rowStore = "San Francisco";
...
function xlsxToXolaRows(workbook, store) {
  ...
  const COUPON_CODE_COL = colIdx("Coupon Code");
  const PAY_COL = colIdx("Payment Status");
  const STATUS_COL = colIdx("Status");
  // "Guests" was renamed to "Total Demographics" in newer Xola exports
...
const Tip = ({ active, payload, label }) => {
  ...
  // Find gross for calculating discount %
  const grossEntry = payload.find(p => (p.name||"").toLowerCase() === "gross");
  const grossVal = grossEntry ? Number(grossEntry.value) : 0;
  return /* @__PURE__ */ React.createElement("div", { style: { background: "#1A1A2E", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "10px 14px", fontFamily: "'DM Mono',monospace", fontSize: 12 } },
    /* @__PURE__ */ React.createElement("div", { style: { color: "#666", marginBottom: 4 } }, label),
    payload.map((p, i) => {
      var _a;
      const isDiscount = (p.name||"").toLowerCase().includes("discount");
      const fmt = typeof p.value === "number" && ((_a = p.name) == null ? void 0 : _a.toLowerCase().match(/rev|total|price|gross|actual|discount/)) ? `$${Number(p.value).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}` : Number(p.value).toLocaleString();
      const discPct = isDiscount && grossVal > 0 ? ` (${(Number(p.value)/grossVal*100).toFixed(1)}%)` : "";
      return /* @__PURE__ */ React.createElement("div", { key: i, style: { color: p.color || ACCENT } }, p.name, ": ", fmt, discPct);
    })
  );
};
...
function BrandDashboard({ data, accent, allStores, label }) {
  ...
  React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 } },
    ...
    [...matchRevel, ...matchResova.map((r) => __spreadProps(__spreadValues({}, r), { name: r.item, units: r.qty || 1 }))].forEach((r) => {
      ...
      if (!byMonth[key]) byMonth[key] = { month: label2, key, revenue: 0, units: 0, gross: 0 };
      byMonth[key].revenue += r.revenue;
      byMonth[key].gross += (r.gross||r.revenue||0);
      byMonth[key].units += r.units || 1;
    });
    const monthData = Object.values(byMonth).sort((a, b) => a.key.localeCompare(b.key)).map((m) => ({...m, revenue: +m.revenue.toFixed(2), gross: +(m.gross||m.revenue).toFixed(2), discount: +((m.gross||m.revenue) - m.revenue).toFixed(2)}));
    const byStore = {};
    [...matchRevel, ...matchResova.map((r) => __spreadProps(__spreadValues({}, r), { name: r.item, units: r.qty || 1 }))].forEach((r) => {
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(
      ...
      return 0;
            ...
            ...[
              ["store","Store"],["units","Units"],["gross","Gross"],["revenue","Actual Rev"],["discPct","Disc %"],["avg","Avg/Unit"],["share","Share"]
            ...
            /* @__PURE__ */ React.createElement("tbody", null, [...agg.byStore].sort((a,b) => {
              ...
              if(sortStoreCol==="discPct") { const aPct=(a.gross>0?(a.gross-a.revenue)/a.gross:0); const bPct=(b.gross>0?(b.gross-b.revenue)/b.gross:0); return mul*(aPct-bPct); }
              ...
              return 0;
            }).map((s, i) => /* @__PURE__ */ React.createElement("tr", { key: s.store }, /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: "#333" } }, i + 1), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: "#ddd" } }, s.store), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: TEAL } }, s.units), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: "#22C55E" } }, "$", (s.gross||s.revenue).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: "#F97316" } }, "$", s.revenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: (s.gross&&s.gross>s.revenue)?"#F59E0B":"#444", fontSize:10 } }, s.gross&&s.gross>0?((s.gross-s.revenue)/s.gross*100).toFixed(1)+"%":"0%"), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", color: GOLD } }, "$", s.units ? (s.revenue / s.units).toFixed(2) : "\u2014"), /* @__PURE__ */ React.createElement("td", { style: { padding: "8px 10px", minWidth: 100 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1, height: 3, background: BORDER, borderRadius: 2 } }, /* @__PURE__ */ React.createElement("div", { style: { width: `${Math.round(s.revenue / agg.totalRevenue * 100)}%`, height: "100%", background: COLORS[i % 10], borderRadius: 2 } })), /* @__PURE__ */ React.createElement("span", { style: { color: "#444", fontSize: 10 } }, Math.round(s.revenue / agg.totalRevenue * 100), "%"))))))),
              /* Totals row */
              (() => {
                const totGross = agg.byStore.reduce((s,x)=>s+(x.gross||x.revenue||0),0);
                const totRev = agg.byStore.reduce((s,x)=>s+x.revenue,0);
                const totUnits = agg.byStore.reduce((s,x)=>s+x.units,0);
                const totDiscPct = totGross>0?((totGross-totRev)/totGross*100).toFixed(1)+"%" :"0%";
                return /* @__PURE__ */ React.createElement("tr", { style: { borderTop: "2px solid #333", background: "#111118" } },
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#555", fontSize: 9 } }, "—"),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#888", fontSize: 11, fontWeight: 600, letterSpacing: 1 } }, "TOTAL"),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: TEAL, fontWeight: 600 } }, totUnits.toLocaleString()),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#22C55E", fontWeight: 600 } }, "$", totGross.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#F97316", fontWeight: 600 } }, "$", totRev.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#F59E0B", fontWeight: 600, fontSize: 12 } }, totDiscPct),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: GOLD, fontWeight: 600 } }, "$", totUnits?(totRev/totUnits).toFixed(2):"—"),
                  /* @__PURE__ */ React.createElement("td", { style: { padding: "9px 10px", color: "#444" } }, "100%")
                );
              })()
            )), subtab === "storedetail" && /* @__PURE__ */ React.createElement("div", { className: "fu" }, /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 16 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, textTransform: "uppercase" } }, "Select Stores ", selectedStore && selectedStore.length > 0 ? `\u2014 ${selectedStore.length} selected` : ""), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10 } }, /* @__PURE__ */ React.createElement(
    "button",
    {
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { style: { background: CARD, border: `1px solid ${accent}33`, borderRadius: 10, padding: "14px 20px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: "'Bebas Neue'", fontSize: 22, color: "#fff", letterSpacing: 2 } }, title), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 20 } }, /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase" } }, "Gross"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, color: "#22C55E", fontFamily: "'Bebas Neue'" } }, "$", totalGrossStore.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}))), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase" } }, "Actual"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, color: "#F97316", fontFamily: "'Bebas Neue'" } }, "$", totalRev.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}))), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase" } }, "Units"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, color: GOLD, fontFamily: "'Bebas Neue'" } }, totalUnits.toLocaleString())), /* @__PURE__ */ React.createElement("div", { style: { textAlign: "right" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase" } }, "Products"), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 16, color: TEAL, fontFamily: "'Bebas Neue'" } }, items.length)))), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "2fr 1fr", gap: 14, marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { background: CARD, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 20 } }, (()=>{
        const chartToggle = /* @__PURE__ */ React.createElement("div", { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 10 } },
          /* @__PURE__ */ React.createElement("div", { style: { fontSize: 10, color: "#555", letterSpacing: 2, textTransform: "uppercase" } }, "Monthly Revenue"),
          /* @__PURE__ */ React.createElement("div", { style: { display:"flex", gap:6 } },
            /* @__PURE__ */ React.createElement("button", { onClick:()=>setStoreDetailChartMode("revenue"), style:{ padding:"3px 10px", borderRadius:5, background:storeDetailChartMode==="revenue"?accent+"33":"transparent", border:`1px solid ${storeDetailChartMode==="revenue"?accent:"#1E1E2E"}`, color:storeDetailChartMode==="revenue"?accent:"#555", fontSize:10, cursor:"pointer", fontFamily:"'DM Mono',monospace" } }, "REVENUE"),
            /* @__PURE__ */ React.createElement("button", { onClick:()=>setStoreDetailChartMode("discount"), style:{ padding:"3px 10px", borderRadius:5, background:storeDetailChartMode==="discount"?"#F59E0B33":"transparent", border:`1px solid ${storeDetailChartMode==="discount"?"#F59E0B":"#1E1E2E"}`, color:storeDetailChartMode==="discount"?"#F59E0B":"#555", fontSize:10, cursor:"pointer", fontFamily:"'DM Mono',monospace" } }, "DISCOUNTED")
          )
        );
        const discSummary = storeDetailChartMode==="discount" && totalGrossStore>0
          ? /* @__PURE__ */ React.createElement("div", { style:{ marginBottom:8, fontSize:12, color:"#F59E0B" } },
              ((totalGrossStore-totalRev)/totalGrossStore*100).toFixed(1)+"% discounted — $"+(totalGrossStore-totalRev).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})+" off $"+totalGrossStore.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})
            )
          : null;
        const chartEl = monthData.length === 0
          ? /* @__PURE__ */ React.createElement("div", { style: { color: "#333", fontSize: 11 } }, "No data in range")
          : /* @__PURE__ */ React.createElement(ResponsiveContainer, { width:"100%", height:180 },
              /* @__PURE__ */ React.createElement(BarChart, { data: monthData },
                /* @__PURE__ */ React.createElement(CartesianGrid, { strokeDasharray:"3 3", stroke:"#1A1A2A" }),
                /* @__PURE__ */ React.createElement(XAxis, { dataKey:"month", tick:{ fill:"#555", fontSize:9 }, axisLine:false, tickLine:false }),
                /* @__PURE__ */ React.createElement(YAxis, { tick:{ fill:"#555", fontSize:9 }, axisLine:false, tickLine:false, tickFormatter:v=>"$"+(v/1e3).toFixed(1)+"K" }),
                /* @__PURE__ */ React.createElement(Tooltip, { content:/* @__PURE__ */ React.createElement(Tip, null) }),
                storeDetailChartMode==="revenue"
                  ? /* @__PURE__ */ React.createElement(React.Fragment, null,
                      /* @__PURE__ */ React.createElement(Bar, { dataKey:"gross", fill:"#22C55E", opacity:0.5, radius:[0,0,0,0], name:"Gross" }),
                      /* @__PURE__ */ React.createElement(Bar, { dataKey:"revenue", fill:accent, radius:[3,3,0,0], name:"Actual" })
                    )
                  : /* @__PURE__ */ React.createElement(Bar, { dataKey:"discount", fill:"#F59E0B", radius:[3,3,0,0], name:"Discounted" })
              )
            );
        return /* @__PURE__ */ React.createElement(React.Fragment, null, chartToggle, discSummary, chartEl);
  ...
  function getCategory(name, brand) {
    ...
    // Misc — ONLY these specific named items go here
    ...
    // Everything else goes to Beverages
    return "Beverages";
  }
