...
async function saveToSheets(url, data) {
  ...
  // Then append all resova rows in chunks
...
function aggregate(revelRows, resovaRows, stores, dateFrom, dateTo) {
  ...
  rr.forEach(r=>{
    ...
    const label=mk!=="Unknown"?`${MONTHS[parseInt(mk.slice(5,7))-1]} '${mk.slice(2,4)}`:mk;
  ...
  return{
    ...
    monthlyRevenue:Object.values(byMonth).sort((a,b)=>a.key<b.key?-1:1).map(m=>({...m,revenue:+m.revenue.toFixed(2)})),
    topItems,
  ...
  const upload=async()=>{
    try{
      ...
      files.forEach(f=>{
        ...
        if(f.type==="revel")  revelRows=[...revelRows,...csvToRevelRows(f.rows,store,fallbackDate)];
      ...
      // Save locally first so data is never lost even if Sheets fails
      saveLocal(newData);
      setStatus("saving");
      if(sheetsUrl){
        try{ await saveToSheets(sheetsUrl,newData); }
